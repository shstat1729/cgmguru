...
// Optimized Level1HypoglycemicEvents calculator class
// Detects events in the range: 54 <= glucose <= 69 mg/dL
class OptimizedLevel1HypoglycemicEventsCalculator : public IdBasedCalculator {
private:
  ...
  // Optimized Level 1 hypoglycemic event detection for a single ID (stays within ID boundaries)
  // Level 1: 54 <= glucose <= 69 mg/dL
  IntegerVector calculate_level1_hypo_events_for_id(const NumericVector& time_subset,
                                                  const NumericVector& glucose_subset,
                                                  int min_readings,
    ...
    if (n_subset == 0) return level1_hypo_events_subset;

    bool level1_hypo_event = false;
    int start_idx = -1;
    int j = 0;
    ...
    while (j < n_subset) {
      // Check for data gap > (end_length + small tolerance) minutes - end any ongoing event
      ...
      if (j < n_subset - 1 && (time_subset[j+1] - time_subset[j]) > gap_threshold_secs) {
        if (level1_hypo_event && start_idx != -1) {
          level1_hypo_events_subset[j] = -1;
          level1_hypo_event = false;
          start_idx = -1;
        }
        j++;
        continue;
      ...
      if (!level1_hypo_event) {
        ...
        if (valid_glucose[j] && glucose_values[j] >= start_gl_min && glucose_values[j] <= start_gl_max) {
          ...
          if (validate_level1_hypoglycemic_event_optimized(time_subset, valid_glucose, glucose_values,
            ...
            level1_hypo_event = true;
            start_idx = j;
            level1_hypo_events_subset[start_idx] = 2; // Event start marker
          }
        }
      } else {
        // Currently in Level 1 hypoglycemic event, look for end condition
        // Event ends ONLY when there is ≥15 consecutive min with CGM sensor value of ≥70 mg/dL
        if (valid_glucose[j] && glucose_values[j] >= end_gl) {
          ...
          if (validate_event_end_optimized(time_subset, valid_glucose, glucose_values,
            ...
            level1_hypo_events_subset[j] = -1; // Event end marker
            level1_hypo_event = false;
            start_idx = -1;
          }
        }
        // Note: No early termination for glucose < 54 or glucose > 69
        // Level 1 events continue until proper recovery (≥70 mg/dL for ≥15 min)
      }

      j++;
    }

    return level1_hypo_events_subset;
  }

  // Optimized Level 1 hypoglycemic event validation (stays within the same ID's data)
  // Only count time when glucose is between 54 <= gl <= 69
  bool validate_level1_hypoglycemic_event_optimized(const NumericVector& time_subset,
    ...
    // Allow small tolerance for irregular sampling intervals (e.g., 4.9833 ≈ 5)
    const double epsilon_minutes = 0.1; // 6 seconds tolerance
    bool reached_duration = accumulated_time + epsilon_minutes >= dur_length;
  ...
  bool validate_event_end_optimized(const NumericVector& time_subset,
    ...
    while (k < n_subset && time_subset[k] <= target_end_time) {
      if (valid_glucose[k] && glucose_values[k] < end_gl) {
        return false; // Glucose went back below end_gl
      }
      k++;
    }

    // Check if we maintained recovery for sufficient time
    // If there were no readings within the target window (k == recovery_start + 1),
    // assume recovery was maintained (no evidence of drop below threshold)
    bool reached_duration = (k > recovery_start + 1) ?
      (time_subset[k - 1] - recovery_start_time) >= end_length * 60 : true;
  ...
  void process_events_with_total_optimized(const std::string& current_id,
                                         const IntegerVector& level1_hypo_events_subset,
                                         const NumericVector& time_subset,
                                         const NumericVector& glucose_subset,
                                         double start_gl_min, double start_gl_max) {
    // First do the standard episode processing
    process_episodes(current_id, level1_hypo_events_subset, time_subset, glucose_subset);
    ...
    size_t estimated_events = std::count(level1_hypo_events_subset.begin(), level1_hypo_events_subset.end(), 2);
    ...
    for (int i = 0; i < level1_hypo_events_subset.length(); ++i) {
      if (level1_hypo_events_subset[i] == 2) {
        // Event start
        start_idx = i;
      } else if (level1_hypo_events_subset[i] == -1 && start_idx != -1) {
  ...
  DataFrame create_level1_hypo_events_total_df() const {
  ...
  DataFrame create_events_total_df(const StringVector& all_ids, const IntegerVector& Elevel1hypoE_final) const {
    ...
    for (int i = 0; i < Elevel1hypoE_final.length(); ++i) {
      if (Elevel1hypoE_final[i] == 2) {  // Event start marker
        std::string id_str = as<std::string>(all_ids[i]);
...
public:
  OptimizedLevel1HypoglycemicEventsCalculator() {
  ...
  List calculate_with_parameters(const DataFrame& df,
    ...
    std::map<std::string, IntegerVector> id_level1_hypo_results;

    // Calculate Level 1 hypoglycemic events for each ID separately to ensure proper boundaries
    for (auto const& id_pair : id_indices) {
      std::string current_id = id_pair.first;
      const std::vector<int>& indices = id_pair.second;
      ...
      // Calculate Level 1 hypoglycemic events for this ID only
      IntegerVector level1_hypo_events_subset = calculate_level1_hypo_events_for_id(time_subset, glucose_subset,
                                                                            min_readings, dur_length, end_length,
                                                                            start_gl_min, start_gl_max, end_gl);

      // Store result
      id_level1_hypo_results[current_id] = level1_hypo_events_subset;

      // Process events for this ID (both standard and total)
      process_events_with_total_optimized(current_id, level1_hypo_events_subset, time_subset, glucose_subset, start_gl_min, start_gl_max);
    }

    // --- Step 4: Merge results back to original order ---
    IntegerVector level1_hypo_events_final = merge_results(id_level1_hypo_results, n);

    // --- Step 5: Create output structures ---
    DataFrame level1_hypo_events_total_df = create_level1_hypo_events_total_df();
    DataFrame events_total_df = create_events_total_df(id, level1_hypo_events_final);

    // --- Step 6: Return final results ---
    return List::create(
      _["events_total"] = events_total_df,
      _["events_detailed"] = level1_hypo_events_total_df
    );
  }
};
